...
uses
  ...
  type
    ...
    public
      ...
      procedure CollectSpritesFrom(ASource: TMap);
...
procedure TMap.CollectSpritesFrom(ASource: TMap);
var
...
begin
  if (fSize.cx = 0)
  then SetSize(TSize.Create(COLLECTION_MAX_ROW_SIZE, fSize.cy));
  ...
  do begin
    ...
    do begin
      if not ASource.CellAt(TPoint.Create(J, I)).Ignore
        and not ContainsSprite(ASource.SpriteAt(TPoint.Create(J, I)))
      then AddSprite(ASource.SpriteAt(TPoint.Create(J, I)), iCurrentSprite);
...
var
  I: Integer;
begin
  for I := fMap.Count - 1 downto 0
  do DeleteRow(I);

  fGraphicAnalog.SetSize(0, 0);
end;
...
begin
  ...
  then begin
    SetSize(TSize.Create((AGraphicAnalog.Width div CELL_SIZE) + 1,
                         (AGraphicAnalog.Height div CELL_SIZE) + 1));
...
begin
  With fSpriteBuffer.Canvas do
  begin
    Pen.Style := psSolid;
    Pen.Color := SPRITE_EMPTY_COLOR;
    Brush.Style := bsSolid;
    Brush.Color := SPRITE_EMPTY_COLOR;
    Rectangle(0, 0, fSpriteBuffer.Width, fSpriteBuffer.Height);
  end;

  SetSprite(APosition, fSpriteBuffer);
end;
